...
export async function GET() {
    ...
    //
    // return NextResponse
    //     .json({ message: "Hello refresh token" },
    //     {
    //         status: 200
    //     });
...
//     If the refresh token is not found, make a POST request to the API to get a new refresh token
    const response = await fetch(`${process.env.DJANGO_API_URL}/api/token/refresh/`, {
        method: "POST",
        headers: {
